---
description: "Use when editing reel format pipelines (Format A, Format B). Covers format isolation, routing chain, design models, rendering, and how to add new formats."
...
Two independent reel pipelines. Each brand has `reel_format` (`"format_a"` or `"format_b"`) stored in `toby_brand_config`.
...
## Format A — `format_a` (light/dark variants)
...
## Format B — `format_b` (slideshow with text header)

| Layer | File | Key Entry Point |
|---|---|---|
| Design Editor | `src/features/reels/DesignEditorTab.tsx` → `FormatBDesign` | Font, gap, padding, image height sliders |
| Design Storage | `app/models/format_b_design.py` → `FormatBDesign` model | `format_b_design` table per user |
| API Routes | `app/api/content/format_b_routes.py` | `/discover`, `/polish`, `/source-images`, `/generate` |
...
| Job Routing | `app/services/content/job_processor.py` → `process_format_b_brand()` | `variant == "format_b"` |
| Toby Pool | `app/services/toby/learning_engine.py` → `FORMAT_B_PERSONALITIES` | `content_type == "format_b_reel"` |

## Routing Chain (how format propagates end-to-end)

1. **Brand config** → `toby_brand_config.reel_format` = `"format_a"` or `"format_b"`
2. **Buffer manager** (`buffer_manager.py`) → reads `bc.reel_format`, maps to `content_type` = `"reel"` or `"format_b_reel"`
3. **Orchestrator** (`orchestrator.py`) → branches on `plan.content_type` → different content generators
4. **Job creation** (`orchestrator.py`) → sets `content_format` and `variant` on `GenerationJob`
5. **Job processor** (`job_processor.py`) → uses `VARIANT_PROCESSORS` registry to route `job.variant` → correct renderer
...
- **Separate API route files.** Format A: `reel_routes.py`. Format B: `format_b_routes.py`. Never mix endpoints.
...
- **Separate design models.** Format A: `brands.colors`. Format B: `format_b_design` table. Never read one format's design settings when rendering the other.
- **Separate personality pools.** `REEL_PERSONALITIES` vs `FORMAT_B_PERSONALITIES`. Never mix pools.
- **Job processor routing via registry.** `VARIANT_PROCESSORS` dict maps variant → method. Never rely on fallthrough or default behavior.

## Adding a New Format (e.g., Format C)

Follow this checklist in order:

1. **DB migration:** Add `"format_c"` to `reel_format` CHECK constraint
2. **Design model:** Create new model + table (like `FormatBDesign`)
3. **Design editor:** Add new tab in `DesignEditorTab.tsx` with its own component
4. **API routes:** Create new route file (like `format_b_routes.py`), register in `main.py`
...
10. **Job processor:** Add entry to `VARIANT_PROCESSORS` registry + implement the method
