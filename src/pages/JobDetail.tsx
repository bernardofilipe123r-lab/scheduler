        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className="text-sm font-mono text-gray-500 bg-gray-800 px-2 py-0.5 rounded">#{job.id}</span>
              <StatusBadge status={job.status} size="md" />
              {isTextVideo && (
                <span className="text-xs font-medium bg-purple-600/30 text-purple-300 px-2 py-0.5 rounded-full border border-purple-500/30">FORMAT B</span>
              )}
            </div>
            {isAutoGenerated ? (
              ...
              <>
                ...
                {job.content_lines && job.content_lines.length > 0 && (
                  <div className="mt-3 text-sm text-gray-300 bg-gray-800/50 rounded-lg p-3 border border-gray-700/50">
                    <p className="font-medium text-gray-200 mb-1">Content:</p>
                    {isTextVideo ? (
                      <p className="whitespace-pre-line">{job.content_lines.join('\n')}</p>
                    ) : (
        ...
        <div className="flex items-center gap-2 flex-shrink-0 whitespace-nowrap">
          {!isAutoGenerated && (
            <button
              ...
              className="px-4 py-2 text-sm font-medium rounded-lg bg-white/10 text-gray-300 hover:bg-white/20 border border-gray-600/50 transition-colors flex items-center gap-2"
          ...
          <button
            ...
            className="px-4 py-2 text-sm font-medium rounded-lg bg-red-500/20 text-red-400 hover:bg-red-500/30 border border-red-500/30 transition-colors flex items-center gap-2"
          >
                  ...
                  <div className="space-y-4">
                    ...
                    {isTextVideo && job.text_video_data && (
                      <div className="bg-gray-900 rounded-lg border border-gray-700 overflow-hidden">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-mono bg-purple-600/30 text-purple-300 px-2 py-0.5 rounded">AI</span>
                        ...
                        {showPromptDetails && (
                          <div className="px-4 pb-4 space-y-4 border-t border-gray-700">
                            ...
                            {(job.text_video_data as any).deepseek_response && (
                              <div className="mt-3">
                                <p className="text-xs font-medium text-blue-400 mb-1.5">DeepSeek Full Response:</p>
                                <pre className="text-xs text-gray-400 bg-gray-950 rounded p-3 overflow-x-auto whitespace-pre-wrap max-h-[300px] overflow-y-auto font-mono">
                                  {(job.text_video_data as any).deepseek_response}
                                </pre>
                              </div>
                            )}
                            ...
                            {((job.text_video_data as any).images || []).length > 0 && (
                              <div>
                                <p className="text-xs font-medium text-green-400 mb-1.5">DeAPI Image Prompts (Flux1schnell):</p>
                                <div className="space-y-2">
                                  {((job.text_video_data as any).images as { query: string; source_type: string }[]).map((img, idx) => (
                                    <div key={idx} className="text-xs bg-gray-950 rounded p-2.5 border-l-2 border-green-600">
                                      <span className="text-green-500 font-medium">Image {idx + 1}:</span>
                                      <span className="text-gray-400 ml-2">{img.query}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>
                            ...
                            {(job.text_video_data as any).thumbnail_image?.query && (
                              <div>
                                <p className="text-xs font-medium text-amber-400 mb-1.5">Thumbnail Image Prompt:</p>
                                <div className="text-xs bg-gray-950 rounded p-2.5 border-l-2 border-amber-600 text-gray-400">
                                  {(job.text_video_data as any).thumbnail_image.query}
                                </div>
                              </div>
                            )}
