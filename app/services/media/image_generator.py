...
class ImageGenerator:
    ...
    def __init__(self, brand_type, variant: str = "light", brand_name: str = "default", ai_prompt: str = None, content_context: str = None, image_model: str = None, ctx=None):
        """
    ...
    def _get_brand_handle(self) -> str:
        """Look up brand handle for CTA placeholder replacement in reel text."""
        try:
            from app.db_connection import get_db_session
            with get_db_session() as db:
                from app.models.brands import Brand
                brand = db.query(Brand).filter(Brand.brand_name == self.brand_name).first()
                if brand and getattr(brand, "instagram_handle", None):
                    handle = brand.instagram_handle.strip()
                    return handle if handle.startswith("@") else f"@{handle}"
        except Exception:
            pass
        return f"@{self.brand_name}" if self.brand_name else "our page"
    ...
    def generate_reel_image(
        ...
        if cta_type or ctx:
            if cta_type:
                ...
                cta_line = get_cta_line(ctx, brand_handle=self._get_brand_handle())
            if cta_line:
                lines = lines.copy()
