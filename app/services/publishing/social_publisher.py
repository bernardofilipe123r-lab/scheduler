...
class SocialPublisher:
    ...
    def publish_threads_post(
        ...
        threads_user_id = getattr(self, '_brand_config', None) and self._brand_config.threads_user_id
        access_token = getattr(self, '_brand_config', None) and self._brand_config.threads_access_token

        if not threads_user_id or not access_token:
            return {"success": False, "error": "Threads not connected", "platform": "threads"}
        ...
        try:
            ...
            # Step 2: Poll for processing (especially for video)
            if media_type == "VIDEO":
                self._poll_threads_status(creation_id, access_token)
            ...
            thread_id = pub_resp.json()["id"]

            print(f"   ✅ Threads post published: {thread_id}")
            return {"success": True, "thread_id": thread_id, "platform": "threads"}

        except Exception as e:
            print(f"   ❌ Threads publish failed: {e}")
            return {"success": False, "error": str(e), "platform": "threads"}

    def _poll_threads_status(self, creation_id: str, token: str, timeout_s: int = 120):
    ...
    def publish_tiktok_video(
        ...
        brand_config = getattr(self, '_brand_config', None)
        if not brand_config or not brand_config.tiktok_refresh_token:
            return {"success": False, "error": "TikTok not connected", "platform": "tiktok"}

        try:
            # Refresh access token first (expires every 24h)
            fresh_token = self._refresh_tiktok_token()

            # Initialize video upload (PULL_FROM_URL)
            ...
            print(f"   ✅ TikTok video upload initiated: {publish_id}")
            return {"success": True, "publish_id": publish_id, "platform": "tiktok"}

        except Exception as e:
            print(f"   ❌ TikTok publish failed: {e}")
            return {"success": False, "error": str(e), "platform": "tiktok"}

    def _refresh_tiktok_token(self) -> str:
        ...
        tokens = svc.refresh_access_token(self._brand_config.tiktok_refresh_token)

        db = SessionLocal()
        try:
            brand = db.query(BrandModel).filter(BrandModel.name == self._brand_config.name).first()
            if brand:
                brand.tiktok_access_token = tokens["access_token"]
                brand.tiktok_access_token_expires_at = datetime.now(timezone.utc) + timedelta(seconds=tokens.get("expires_in", 86400))
