...
class SocialPublisher:
    ...
    def _ensure_bsky_session(self) -> Optional[str]:
        ...
        # Try refreshing if we have a refresh JWT
        ...
        # Fall back to creating a new session with app password
        try:
    ...
    def _truncate_to_graphemes(text: str, max_graphemes: int = 300) -> str:
        ...
        while i < len(text) and len(graphemes) < max_graphemes:
            char = text[i]
            # Simple grapheme clustering: combine base + combining marks
            cluster = char
            i += 1
    ...
    def publish_bsky_post(
        ...
        Args:
            caption: Post text (will be truncated to 300 graphemes)
            media_url: Optional public URL to image or video
            media_type: "TEXT", "IMAGE", or "VIDEO"

        Returns:
            Dict with success, post_id (AT URI), platform
        """
        if not self.bsky_did or not self.bsky_app_password:
            return {
                "success": False,
        ...
        try:
            # Truncate caption to 300 graphemes
            ...
            # Handle video — Bluesky video upload uses a separate service
            elif media_url and media_type == "VIDEO":
