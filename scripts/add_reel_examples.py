"""
Script to add 30 random non-repeated reel examples from the viral ideas database
to the user's niche_config entries.
"""
import os
import json
import random
...
# All 59 viral ideas (title + content_lines only, for reel_examples format)
VIRAL_IDEAS = [
    {
...
def main():
    # Step 1: Get existing reel example titles for ALL configs of this user
    with engine.connect() as conn:
        rows = conn.execute(
            text("SELECT id, reel_examples FROM niche_config WHERE user_id = :uid"),
            {"uid": USER_ID}
        ).fetchall()
    ...
    for row in rows:
        ...
        examples = row[1] or []
        for ex in examples:
            existing_titles.add(ex.get("title", "").strip().upper())
    
    print(f"Found {len(config_ids)} niche_config(s) for user")
    print(f"Existing reel example titles across all configs: {len(existing_titles)}")
    for t in sorted(existing_titles):
        print(f"  - {t}")
    
    # Step 2: Filter out ideas that already exist (by title)
    available_ideas = [
        idea for idea in VIRAL_IDEAS
        if idea["title"].strip().upper() not in existing_titles
    ]
    print(f"\nAvailable (non-duplicate) ideas: {len(available_ideas)}")
    
    # Step 3: Select 30 random from available
    num_to_add = min(30, len(available_ideas))
    selected = random.sample(available_ideas, num_to_add)
    print(f"Selected {num_to_add} new reel examples to add")
    
    # Step 4: For each config, append the 30 new examples to existing reel_examples
    with engine.begin() as conn:
        for config_id in config_ids:
            # Get current examples
            row = conn.execute(
                text("SELECT reel_examples FROM niche_config WHERE id = :cid"),
                {"cid": config_id}
            ).fetchone()
            current_examples = row[0] or []
            
            # Append new ones
            updated_examples = current_examples + selected
            
            # Update
            conn.execute(
                text("UPDATE niche_config SET reel_examples = :examples, updated_at = NOW() WHERE id = :cid"),
                {"examples": json.dumps(updated_examples), "cid": config_id}
            )
            print(f"  Updated config {config_id}: {len(current_examples)} -> {len(updated_examples)} reel examples")
    
    print("\nDone! Added new reel examples to all configs.")
    
    # Step 5: Verify
    with engine.connect() as conn:
        for config_id in config_ids:
            row = conn.execute(
                text("SELECT jsonb_array_length(COALESCE(reel_examples, '[]'::jsonb)) FROM niche_config WHERE id = :cid"),
                {"cid": config_id}
            ).fetchone()
            print(f"  Config {config_id}: {row[0]} reel examples (verified)")
